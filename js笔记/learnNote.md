...
function T1(){//在全局定义
    this.name = "111";
    this.d = function(){//[[Environment]]指向全局
        console.log(i);
    }
}
...

{
    function T2(){//在代码块内定义
        this.name = "111";
        this.d = function(){//[[Environment]]指向代码块
            console.log(i);
        }
    }
...
t2.d();//error，因为代码块内没有嵌套函数，因此代码块不会被引用，执行完里面的代码后里面所有东西都被回收
```
...
{
    let i = 10;//代码块内定义变量i，用于测试
    t.d = function(){//方法在代码块内被创建，[[Environment]]指向代码块
        console.log(i);
    }
    t.d();//10
}

t.d();//10，因为代码块内具有嵌套函数，并且可以被变量t访问，所以代码块不会被回收，并且能说明方法指向代码块
/* *********************************************************************** */

function T1(){//对象中没有任何方法
    this.name = "111";
}

let t1 = new T1();

t.d = function(){//方法在全局被创建，[[Environment]]指向全局
    console.log(i);
}
